...
if __name__ == "__main__":
    ...
    if no_proc_flag:
        ...
        # load validation dataset
            ...
            for split in ["libri-clean", "libri-noisy", "librimix_dev"]:
                ...
                if not data_path.exists():
                    config["datamodule"][split]["dataset"]["path"] = "/home/ab2810/rds/hpc-work/LibriMix"
            ...
            datasets["libri-clean"] = LibriMix(**config["datamodule"]["librimix_dev"]["dataset"])
        ...
        # load model
        model = FastGenSep.load_from_checkpoint(str(args.ckpt))
        ...
        if args.tag is None:
            output_dir = output_dir_base / f"{exp_name}_{ckpt_name}_{N}_steps"
        else:
            output_dir = output_dir_base / f"{args.tag}_{N}steps"
    ...
    for split, dl in dataloaders.items():
        ...
        for batch_idx, (mix, target) in enumerate(dl):
            ...
            if no_proc_flag:
                ...
                x_result = model.separate(mix, torch.Tensor([N]).to(device))
